...
<div class="acrylic-sidebar-fixed">
    <button type="button" class="sidebar-close-btn d-lg-none" id="sidebarClose" aria-label="Tutup menu">
        <i class="bi bi-x-lg"></i>
    </button>
    <div class="text-center mb-4">
        <div class="sidebar-icon-wrapper mb-3" role="button" tabindex="0" aria-label="Profil saya" data-profile-sheet-trigger>
            <i class="bi bi-code-slash fs-1 text-gradient"></i>
        </div>
    ...
    @php
        $isDashboard = request()->routeIs('developer.dashboard');
        $isMyTeam = request()->routeIs('developer.myteam');

        if (!$isDashboard && !$isMyTeam) {
            $isDashboard = true;
        }
    @endphp
    ...
    <ul class="nav flex-column d-flex flex-grow-1">
        ...
        <li class="nav-item mb-2">
            <a href="{{ route('developer.myteam') }}" 
               class="nav-link-acrylic {{ $isMyTeam ? 'active' : '' }}">
